...
def load_aware_greedy_vertex_cut(edges, num_parts):
    pmt = np.random.permutation(len(edges))
    #print(len(pmt))
    lst_parts = [GraphPartition() for i in range(num_parts)]
    A_map = defaultdict(set)
    ...
    for i in pmt:
        ...
        if(len(s)>0):
            ...
            if(len(Au)==0 and len(Av)==0):
                ...
                j = get_min_loaded_from_set(Au, lst_parts)
                ...
                uec_v = unassigned_edge_count[v]
                if(uec_u>=uec_v):
                    j = get_min_loaded_from_set(Au, lst_parts) #Au.pop() 
                else:
                    j = get_min_loaded_from_set(Av, lst_parts)#Av.pop()
            else:
                # Au or Av is empty
                if(len(Au)>0):
                    j = get_min_loaded_from_set(Au, lst_parts)#Au.pop()
